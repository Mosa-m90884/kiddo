...
public class FirebaseManager {
    ...
    public Task<String> signIn(String email, String password,Context context) {
        ...
        return firebaseAuth.signInWithEmailAndPassword(email, password)
                .continueWithTask(task -> {
                    if (task.isSuccessful()) {
                        ...
                        return checkUserInFirestore(email,context);
                    } else {
    ...
    private Task<String> checkUserInFirestore(String email,Context context) {
          preferences =new SharedPreferencesHelper(context);;
        ...
        db.collection("parents")
                ...
                .addOnCompleteListener(task -> {
                    if (task.isSuccessful() && !task.getResult().isEmpty()) {
                        // البريد الإلكتروني ينتمي للآباء
                        taskCompletionSource.setResult("parent");
                        preferences.saveString("username", "parent");
                        ...
                        db.collection("children")
                                ...
                                .addOnCompleteListener(task1 -> {
                                    if (task1.isSuccessful() && !task1.getResult().isEmpty()) {
                                        preferences.saveString("username", "children");
    ...
    public Task<Child> getChildrenId(String childId) {
